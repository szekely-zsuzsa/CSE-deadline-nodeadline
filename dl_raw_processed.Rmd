...
response %>% 
  ...
  count(complete) # the result should equal to the number of participants (57) 
```
...
```{r}
# calculate accuracy of calibration trials for later analysis
calibration_accuracy <- response %>%
...
# save calibration accuracy data
write_tsv(calibration_accuracy, "data/deadline/processed/calibration.tsv")

# calculate accuracy for exclusions
response %>%
...
First, we check if personal deadlines equal to the mean RT-s of the matching calibration sections (only correct answers were used for deadline calibration)

```{r}
response %>% 
  select(participant_id, section, blockId, correct, duration, personalDeadline) %>% 
  group_by(participant_id, section, blockId) %>%
  filter(correct == TRUE,
         section != "practice") %>% 
  mutate(mean_duration = mean(duration, na.rm = TRUE)) %>%
  distinct(participant_id, mean_duration, personalDeadline)
```
...
participant_rt %>% 
  distinct(participant_id, personal_mean_duration, personal_sd_duration, grand_mean_duration, grand_sd_duration)

# the participant with the participant_id of 25 seems to be an outlier, i drop it now by hand
participant_rt <- participant_rt %>% 
  filter(participant_id != 25)

# make calculations again for mean and sd of reaction time
participant_rt <- participant_rt %>%
  group_by(participant_id) %>%
  filter(section == "calibration") %>% 
  mutate(personal_mean_duration = mean(duration, na.rm = TRUE),
...
# exclude the outlier participant spotted earlier
response <- response %>%
  group_by(participant_id) %>%
  filter(!participant_id == "25")
```
...
```{r}
# we can remove more, but I leave it like this for now
testRtAnalysis = testRtAnalysis %>% 
  select(-sender_id,-sender_type,-sender,-timestamp, -testTrialset, -calibrationTrialset)

testAccAnalysis = testRtAnalysis %>% 
  select(-sender_id,-sender_type,-sender,-timestamp, -testTrialset, -calibrationTrialset)
```
